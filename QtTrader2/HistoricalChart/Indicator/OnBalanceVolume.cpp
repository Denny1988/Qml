...
{
	OnBalanceVolume::OnBalanceVolume(IndicatorSettings& settings, BaseData& baseData, bool needInitialize)
	...
	IndicatorSettings& OnBalanceVolume::initSettings(IndicatorSettings& settings) const
	{
	...
	void OnBalanceVolume::calculate(int beginIndex, int endIndex)
	{
		extendDataBuffer();
		...
		unique_ptr<CRecordset> result(ta->OnBalanceVolume(database.get(), m_inputSeries, volumes.get(), name));
