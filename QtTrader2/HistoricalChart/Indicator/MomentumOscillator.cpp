...
{
	MomentumOscillator::MomentumOscillator(IndicatorSettings& settings, BaseData& baseData, bool needInitialize)
	...
	IndicatorSettings& MomentumOscillator::initSettings(IndicatorSettings& settings) const
	{
	...
	void MomentumOscillator::calculate(int beginIndex, int endIndex)
	{
		...
		TA_RetCode retCode = TA_MOM(beginIndex, endIndex, inputData.get(), period, &outBegin, &outNum, outReal);
