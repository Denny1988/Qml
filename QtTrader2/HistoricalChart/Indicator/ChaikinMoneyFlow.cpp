...
{
	ChaikinMoneyFlow::ChaikinMoneyFlow(IndicatorSettings& settings, BaseData& baseData, bool needInitialize)
	...
	IndicatorSettings& ChaikinMoneyFlow::initSettings(IndicatorSettings& settings) const
	{
	...
	void ChaikinMoneyFlow::calculate(int beginIndex, int endIndex)
	{
		...
		unique_ptr<CRecordset> result(ta->ChaikinMoneyFlow(database.get(), database->getRecordset(), period, name));
