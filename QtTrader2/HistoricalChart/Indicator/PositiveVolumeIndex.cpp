...
{
	PositiveVolumeIndex::PositiveVolumeIndex(IndicatorSettings& settings, BaseData& baseData, bool needInitialize)
	...
	IndicatorSettings& PositiveVolumeIndex::initSettings(IndicatorSettings& settings) const
	{
	...
	void PositiveVolumeIndex::calculate(int beginIndex, int endIndex)
	{
		extendDataBuffer();
		...
		unique_ptr<CRecordset> result(ta->PositiveVolumeIndex(database.get(), m_inputSeries, volumes.get(), name));
