...
{
	WeightedClose::WeightedClose(IndicatorSettings& settings, BaseData& baseData, bool needInitialize)
	...
	IndicatorSettings& HistoricalChartSpace::WeightedClose::initSettings(IndicatorSettings& settings) const
	{
	...
	void WeightedClose::calculate(int beginIndex, int endIndex)
	{
		// 1. extend buffer
		...
		{
			vectOutReal[i] = (highs[i] + lows[i] + closes[i] * 2) / 4;
		}
	}
}