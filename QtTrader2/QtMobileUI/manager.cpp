...
void Manager::init()
{
    ...
    //login
    Common::LoginSetting::getInstance()->createSettingByPath(QCoreApplication::applicationDirPath());
    Common::AppSetting::getInstance()->createSettingByPath(QCoreApplication::applicationDirPath());
    QString host =  QString("10.3.192.1");
    Service::ServiceManager::getInstance()->setServer(host, 4523, host, 4529, false);
    QString loginID = "dem001";
    QString password = "12345678";
    QString language = "CHS";
    Common::CustomerSetting::getInstance()->createSetting(loginID);
