#ifndef QDECIMAL_QDECNUMBER_H
#define QDECIMAL_QDECNUMBER_H
...
public:
	...
	//取最大值
    QDecNumber getMax(const QDecNumber& o, QDecContext* c = 0) const
	...
	//取最小值
    QDecNumber getMin(const QDecNumber& o, QDecContext* c = 0) const
	...
	static QDecNumber round(const QDecNumber& value)
	{
		return QDecNumber::round(value, 0);
	}

	static QDecNumber round(const QDecNumber& value, int precision)
	{
		if(value.toString().contains("."))
		{
			QString qstr(value.toString());
			int indesx = qstr.indexOf(".");
			QDecContext digits;
			digits.setDigits(indesx + precision);
			return value.normalize(&digits);
		}
		return value;
	}

	static QDecNumber fromQString(const QString& value)
	{
		QDecNumber newQDecNumberValue(value.toStdString().c_str());
		return newQDecNumberValue;
	}
