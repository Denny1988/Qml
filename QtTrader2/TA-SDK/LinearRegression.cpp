...
CLinearRegression::~CLinearRegression()
{
  ...
  CRecordset* CLinearRegression::Regression(CDatabase* pData, CField* pSource, int Periods){
  ...
  CRecordset* CLinearRegression::TimeSeriesForecast(CDatabase* pData,
                   CField* pSource, int Periods, const char* Alias){

    CRecordset* Results;

    Results = Regression(pData, pSource, Periods);
